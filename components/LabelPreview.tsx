    ...
    <div className="flex flex-col xl:flex-row gap-12 items-center justify-center w-full min-h-[500px]">
      <div className="flex flex-col gap-6 items-center label-preview-container flex-1 w-full max-w-[420px]">
        <div className={`relative group transition-all duration-300 w-full flex justify-center ${isDesignMode ? 'scale-100' : 'hover:scale-[1.02]'}`}>
            <div className={`absolute -inset-1 rounded-xl blur opacity-25 group-hover:opacity-50 transition duration-500 ${theme === ThemeType.FLEXI ? 'bg-gradient-to-r from-amber-500 to-orange-600' : 'bg-gradient-to-r from-brand-cyan to-brand-purple'}`}></div>
            <div className="relative w-full bg-slate-900 border border-white/10 p-8 rounded-xl flex flex-col items-center shadow-2xl">
                 ...
                 <div className={`relative bg-white transition-all duration-300 ${isDesignMode ? 'ring-2 ring-brand-cyan ring-offset-4 ring-offset-slate-900 cursor-text shadow-none' : 'shadow-[0_0_30px_rgba(255,255,255,0.1)]'}`}>
                    <div className="printable-label overflow-hidden bg-white text-black" style={sheetStyle}>
                 ...
                 <div className="mt-6 w-full flex justify-between items-center text-xs font-mono text-slate-500 no-print">
                    <div className="flex items-center gap-1"><Printer className="w-3 h-3" /><span>80x60mm</span></div>
                    <div className={theme === ThemeType.FLEXI ? 'text-amber-500' : 'text-brand-cyan'}>{theme === ThemeType.FLEXI ? 'INDUSTRIAL' : 'RETAIL'}</div>
                 </div>
            </div>
        </div>
        ...
        <div className={`relative group transition-all duration-300 w-full flex justify-center ${isDesignMode ? 'scale-100' : 'hover:scale-[1.02]'}`}>
            ...
            <div className="relative w-full bg-slate-900 border border-white/10 p-8 rounded-xl flex flex-col items-center shadow-2xl">
                <div className={`relative bg-white transition-all duration-300 ${isDesignMode ? 'ring-2 ring-brand-purple ring-offset-4 ring-offset-slate-900' : 'shadow-[0_0_30px_rgba(255,255,255,0.1)]'}`}>
                    <div className="printable-label bg-white text-black overflow-hidden flex flex-col items-center justify-center text-center p-4" style={sheetStyle}>
                        ...
                        <div className="mt-3 border border-black rounded px-2 py-1 text-[7pt] font-bold max-w-full">{getQrFooter()}</div>
                    </div>
                </div>
                 <div className="mt-6 w-full flex justify-between items-center text-xs font-mono text-slate-500 no-print">
                    <div className="flex items-center gap-1"><Eye className="w-3 h-3" /><span>Quick Scan</span></div>
